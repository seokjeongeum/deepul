...
hist = Histogram(d)
train_epochs(hist, train_loader, None, dict(epochs=50, lr=2.5e-2))
...
class MixtureOfLogistics(nn.Module):
    ...
    def nll(self, x):
        ...
        log_cdf_plus = plus_in - F.softplus(plus_in) # All probability at tails <= 0
        log_cdf_min = -F.softplus(min_in) # All probability at tails >= d - 1
